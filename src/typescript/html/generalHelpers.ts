...
/// <reference path="../styles/css.ts" />
/// <reference path="../objectHelpers/objects.ts" />
...
namespace KIP {
   ...
   */
     ...
     */
    export function appendChildren(parent: StandardElement, ...kids: StandardElement[]): void {
        ;
        let idx: number;
     ...
     */
    export function clearChildren(parent: StandardElement): void {
        for (let idx = parent.children.length - 1; idx >= 0; idx -= 1) {
            let child: StandardElement = parent.children[idx] as StandardElement;
